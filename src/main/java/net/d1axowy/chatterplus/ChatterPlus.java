...
public final class ChatterPlus extends JavaPlugin implements CommandExecutor, Listener {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("chatclear")) {
            ...
            Player p = (Player) sender;
            if (p.hasPermission("cplus.broadcast")) {
                if (args.length == 0) {
                    p.sendMessage(Utils.color("broadcast-less-args"));
                } else if (args.length >= 1) {
                    ...
                    for (Player target : Bukkit.getOnlinePlayers()) {
                        ...
                        String message = sb.toString().trim();
                        target.sendMessage(Utils.color(getConfig().getString("broadcast-format").replace("%broadcast_msg%", message)));
                    }
    ...
    public void ChatFormatting(AsyncPlayerChatEvent e) {
        if (getConfig().getBoolean("enable-chat-formatting")) {
            ...
            if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null) {
                ...
                e.setMessage(msg);
                e.setFormat(Utils.color(format).replace("%player%", p.getDisplayName()).replace("%message%", msg).replace("%", "%%"));
            } else {
                e.setFormat(Utils.color(format).replace("%player%", p.getDisplayName()).replace("%message%", msg));
            }
        }
    }
}
